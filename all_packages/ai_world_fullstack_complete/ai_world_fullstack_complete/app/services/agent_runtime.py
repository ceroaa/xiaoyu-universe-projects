...
def process_controller_tick(db: Session, controller_id: str) -> dict[str, Any]:
    ...
    if not agent or not actor or agent.status != "ACTIVE":
...
def decide_next_action(db: Session, agent: AIAgent, actor: Actor, profile: AgentProfile | None) -> dict[str, Any] | None:
    room_mates = list(
        db.scalars(
            select(Actor).where(
                Actor.current_room_id == actor.current_room_id,
                Actor.id != actor.id,
                Actor.actor_type == ActorType.HUMAN_CHARACTER,
            )
        ).all()
    )
    ...
    if agent.agent_class.value == "GUARD":
        if random.random() < 0.35:
            return {"action_type": "say", "action_payload": {"content": "北门外最近不太平，若要离村，记得先准备好补给。"}}
        return None

    if agent.agent_class.value == "MERCHANT":
        if random.random() < 0.40:
            return {"action_type": "say", "action_payload": {"content": "晨星酒馆今天的麦酒和口粮都很齐，外出的旅人最好先看看。"}}
        return None
    ...
    if random.random() < 0.30:
        return {"action_type": "say", "action_payload": {"content": "这个村子最近看似平静，但总让人觉得会发生点什么。"}}
    return None
...
def _build_greeting(agent_code: str, actor_name: str, target_name: str) -> str:
    if "elder" in agent_code:
        return f"{target_name}，欢迎来到新手村。若你还在熟悉这里，之后可以来村长小屋找我。"
    if "guard" in agent_code:
        return f"{target_name}，北门外最近不太平。若你要往北走，务必先做好准备。"
    if "merchant" in agent_code:
        return f"{target_name}，要不要先进来坐坐？酒馆里有热食、麦酒和适合旅途的补给。"
    if "patrol" in agent_code:
        return f"{target_name}，最近巡逻变勤了。要是你看见可疑痕迹，记得马上回报。"
    return f"{target_name}，你好，我是 {actor_name}。"
...
def _choose_patrol_move(db: Session, actor: Actor) -> dict[str, Any] | None:
    ...
    return {"direction": str(exit_obj.direction).lower()}
