...
from app.models.narrative import ActorQuest, Quest
from app.models.social import ChatMessage
from app.models.narrative import WorldEvent
from app.models.enums import ChannelType, LocationType, QuestStatus
...
class WorldEngine:
    ...
    def _handle_look(self, actor: Actor) -> EngineResult:
        ...
        visible = {
            'room': {'id': str(room.id), 'code': room.code, 'name': room.name, 'description': room.description},
            ...
            'exits': [e.direction for e in exits],
        }
        return EngineResult(
            result_type='SUCCESS',
    ...
    def _handle_move(self, actor: Actor, payload: dict[str, Any]) -> EngineResult:
        ...
        exit_obj = self._get_exit(actor.current_room_id, direction)
        ...
        return EngineResult(
            result_type='SUCCESS',
            visible_result={
                'message': f'你往 {direction} 移動到 {to_room.name}。',
                'look': look_result.visible_result,
            },
            canonical_result={'action': 'move', 'direction': direction, 'to_room_id': str(to_room.id)},
            emitted_events=look_result.emitted_events,
        )
    ...
    def _handle_say(self, actor: Actor, payload: dict[str, Any]) -> EngineResult:
        ...
        self._create_world_event(
            room_id=actor.current_room_id,
            event_type='CHAT',
            public_text=f'{actor.display_name} 說：「{content}」',
            canonical={'actor_id': str(actor.id), 'channel': 'SAY', 'content': content},
        )
        return EngineResult(
            result_type='SUCCESS',
            visible_result={'message': f'你說：「{content}」'},
    ...
    def _handle_whisper(self, actor: Actor, payload: dict[str, Any]) -> EngineResult:
        ...
        return EngineResult(
            result_type='SUCCESS',
            visible_result={'message': f'你悄悄對 {target.display_name} 說：「{content}」'},
    ...
    def _handle_take(self, actor: Actor, payload: dict[str, Any]) -> EngineResult:
        ...
        return EngineResult(
            result_type='SUCCESS',
            visible_result={'message': f'你拿起了 {item.template.name}。'},
    ...
    def _handle_drop(self, actor: Actor, payload: dict[str, Any]) -> EngineResult:
        ...
        item.location_id = str(actor.current_room_id)
    ...
    def _handle_accept_quest(self, actor: Actor, payload: dict[str, Any]) -> EngineResult:
        ...
        existing = self.db.scalar(
            select(ActorQuest).where(ActorQuest.actor_id == actor.id, ActorQuest.quest_id == quest.id)
        )
        ...
        return EngineResult(
            result_type='SUCCESS',
            visible_result={'message': f'你接受了任務：{quest.title}'},
    ...
    def _get_exit(self, room_id, direction: str) -> RoomExit:
        exit_obj = self.db.scalar(
            select(RoomExit).where(RoomExit.from_room_id == room_id, RoomExit.direction == direction, RoomExit.is_hidden.is_(False))
        )
        if not exit_obj:
    ...
    def _list_room_items(self, room_id) -> list[ItemInstance]:
        stmt = select(ItemInstance).where(ItemInstance.location_type == LocationType.ROOM, ItemInstance.location_id == str(room_id))
    ...
    def _create_world_event(self, room_id, event_type: str, public_text: str, canonical: dict[str, Any]) -> None:
        event = WorldEvent(
            room_id=room_id,
